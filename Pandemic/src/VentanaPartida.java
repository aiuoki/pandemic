...
/**
 * 
 * Esta es la clase de la ventana partida
 * @author aiuoki
 *
 */
public class VentanaPartida extends JFrame implements ActionListener {
	...
	public VentanaPartida() {
		...
		JProgressBar barraAlfa = new JProgressBar();
		...
		JProgressBar barraBeta = new JProgressBar();
		...
		JProgressBar barraGama = new JProgressBar();
		...
		JProgressBar barraDelta = new JProgressBar();
    ...
    /**
     * Metodo encargado de preparar la partida, leer el usuario, leer la dificultad, infectar las ciudades al inicio de la partida...
     */
    static void prepararPartida() {
    	usuario = leerUsuario();
    	...
    	alfa.reiniciarVacuna();
    	beta.reiniciarVacuna();
    	gama.reiniciarVacuna();
    	delta.reiniciarVacuna();
    	
    	for (int i = 0; i < ciudades.size(); i++) {
			ciudades.get(i).reiniciarCiudad();
		}
    	
    	acciones = 4;
    	numEnfermedadesActivas = 0;
    	numBrotes = 0;
    	...
    	infectarCiudadesInicio(ciudades, parametros);
    ...
    /**
     * Metodo leer dificultad, lee el archivo parametros.xml y lee el parametro dificultad
     * @return dificultad. Devuelve la dificultad que pone en los parametros
     */
	static String leerDificultad() {
		...
		try {
			...
			dificultad = elemento.getElementsByTagName("dificultad").item(0).getTextContent();
		} catch(Exception e) {
			System.out.println("\nError E/S: " + e);
		}
	...
	/**
	 * Metodo infectar ciudades inicio, este metodo infecta las ciudades al inicio de la partida
	 ...
	 */
	static ArrayList<Ciudad> infectarCiudadesInicio(ArrayList<Ciudad> ciudades, int [] parametros) {
    ...
    static void guardarPartida() {
		...
		String sql = "INSERT INTO PARTIDA VALUES(pf_seqIdPartida.NEXTVAL, '" + usuario + "', SYSDATE, '" + dificultad + "', '" + estado + "', " + ronda + ", " + acciones + ", " + numEnfermedadesActivas + ", " + numBrotes + ", vacunas(";
		
		sql += "vacuna('" + alfa.getNombre() + "', " + alfa.getNivelInvestigacion() + "), vacuna('" + beta.getNombre() + "', " + beta.getNivelInvestigacion() + "), vacuna('";
		sql += gama.getNombre() + "', " + gama.getNivelInvestigacion() + "), vacuna('" + delta.getNombre() + "', " + delta.getNivelInvestigacion() + ")), ciudades(";
